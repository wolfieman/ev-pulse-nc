...
**What changed:** As of May 2025, NCDOT implemented a new method of tracking registered electric vehicles that (1) eliminates duplicate records and (2) excludes vehicles with plates recently removed from circulation. Source: NCDOT ZEV Registration Data page (https://www.ncdot.gov/initiatives-policies/environmental/climate-change/Pages/zev-registration-data.aspx).

**Impact on this project:** The `Methodology_PostMay2025` boolean is set in `ncdot_ev_pipeline.py` (line 407) for all rows with Date >= 2025-05-01. **The flag is not consumed by any analysis script.** The entire holdout validation period (Jul-Oct 2025) is post-change.

**Interaction with the 69% underprediction:** The new methodology should produce *lower* counts than the old methodology (fewer duplicates, fewer inactive vehicles). Models were trained on pre-change data that included those inflated counts. This means the methodology change alone would push toward *overprediction*, not underprediction. The fact that 69% of forecasts still underpredicted actual values suggests that real EV adoption growth is strong enough to overwhelm the downward correction from deduplication. This actually strengthens the underprediction finding — the growth signal is even stronger than the raw numbers suggest.
...
**MAPE weighting choice:** The overall MAPE (4.34%) is an unweighted flat average across all 400 county-month observations, giving equal importance to forecast accuracy in every county regardless of BEV fleet size. This is both the standard approach in time series forecasting literature and philosophically aligned with the equity-weighted scoring framework — a BEV-weighted MAPE would prioritize accuracy in high-adoption counties, contradicting the 0.40 equity weight that the scoring framework deliberately places on disadvantaged and underserved communities. For context, the BEV-weighted MAPE is 2.63% — the 4.34% headline is conservative because small counties (e.g., Hyde with 4 BEVs, Scotland with 54) produce large percentage errors from small absolute misses. The 10 scoring counties (all 2,000+ BEVs) have substantially lower individual MAPE.
