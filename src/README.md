...
```
src/
├── evpulse/            # Reusable package imported across the scripts
├── analysis/           # Phase 1-5 analytics + scoring framework + EDA
├── data-acquisition/   # API downloads (NCDOT, AFDC, Census, CEJST, LEHD)
├── blog/               # Importable package for blog graphics
├── docs/               # Documentation-asset generators (pipeline diagram)
└── paper/              # Manuscript build script
```

---

## `evpulse/` — shared package

The reusable core the analysis and figure scripts import (editable-installed by `uv sync`). Centralizing it here is what keeps the scripts free of `sys.path` hacks and copy-pasted helpers.

| Module | Provides |
|---|---|
| `paths.py` | `PROJECT_ROOT` — walks up to the repo root, so no script hardcodes `../../..` |
| `constants.py` | Scoring weights, `TARGET_CRS` (EPSG:32119), colormap, study-county list |
| `style.py` | Publication matplotlib defaults (600 DPI, palette) + `save_figure` |
| `io.py` | `load_fips_csv` — the single CSV reader that zero-pads FIPS/ZIP/tract codes |
| `geo.py` | `reproject_gdf` + boundary loaders shared by the geospatial figures |
| `arima.py` | ARIMA modelling core (load / fit / forecast / order-select) behind the BEV forecast |
...
| Group | Scripts | Role |
|---|---|---|
| **Phase 1 — Validation** | `validate_sas_forecasts.py`, `arima_bev_forecast.py`, `arima_template.py`, `generate_phase1_figures.py`, `phase1_fig44_validation_scatter.py` | Cross-validate SAS forecasts in Python (MAPE 4.34%) |
...
| **Phase 5 — Justice40** | `phase5_tract_zcta_crosswalk.py`, `phase5_climate_sensitivity.py`, `phase5_weight_sensitivity.py`, `phase5_fig39_to_fig42.py`, `phase5_fig43_scoring_barchart.py`, `phase5_fig45_archetype_scatter.py` | CEJST overlay + climate / weight sensitivity + scoring figures |
...
| **AFDC EDA (Phase 3)** | `generate_phase3_afdc_eda.py` orchestrates `phase3_afdc_eda_prep.py`, `phase3_afdc_eda_tables.py`, `phase3_afdc_eda_figures.py` | Load/parse AFDC, write tables, build fig-08–17 |
| **EDA** | `eda_cejst_justice40.py`, `eda_census_zip_population.py`, `eda_phase4_lehd_acs.py` | Exploratory profiling per dataset |
...
- `docs/generate_pipeline_diagram.py` — renders the analytical-pipeline architecture diagram into `docs/figures/`.
- `paper/build_docx.py` — builds the manuscript `.docx` from the markdown source (see [`../paper/README.md`](../paper/README.md)).
...
| Module docstring + Copyright banner at the top of every file | [`../STYLE-GUIDE.md`](../STYLE-GUIDE.md) |
...
| Line length 88; `target-version = "py314"`; ruff + pyright clean | [`../pyproject.toml`](../pyproject.toml) |
